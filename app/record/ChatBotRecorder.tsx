...
export default function ChatBotRecorder() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-stone-100 pt-20 pb-8">
      <div className="container mx-auto px-4 max-w-7xl">
        
        {/* 페이지 타이틀 - 카페 페이지 스타일 */}
        <div className="text-center mb-8">
          <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-amber-700 to-orange-800 bg-clip-text text-transparent mb-4">
            📚 My Coffee Journey
          </h1>
          <p className="text-gray-600 text-lg">{user?.displayName || '나'}의 커피 기록</p>
        </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {currentQuestion.options?.map((option) => (
                  <button
                    ...
                    className="p-3 border border-gray-200 rounded-xl hover:border-amber-400 hover:bg-amber-50 transition-all duration-300 text-left text-stone-800"
                  >
                    {option}
                  </button>
                ))}
                    ...
                    <button
                      ...
                      className={`p-3 border rounded-xl transition-all duration-300 text-left ${
                        (recordData.flavor as string[]).includes(option)
                          ...
                          : 'border-gray-200 text-stone-800 hover:border-amber-400 hover:bg-amber-50'
                      }`}
                    >
                      {option}
                    </button>
