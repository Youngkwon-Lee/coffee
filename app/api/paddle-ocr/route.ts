...
function resolvePythonBin() {
  ...
  const candidates = [
    ...
    path.join(home, "projects", "coffee", ".venv-paddle", "bin", "python"),
    "python3",
...
async function runLocalScript(file: File, lang: string) {
  ...
  try {
    ...
    return await runPython(["--image", tempPath, "--lang", lang]);
  } finally {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (PADDLE_OCR_API_URL) {
      ...
      return NextResponse.json(parseJsonSafely(proxied.body), { status: proxied.status });
    }
...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      {
        ...parseJsonSafely(result.stdout || result.stderr),
        mode: "local-python",
